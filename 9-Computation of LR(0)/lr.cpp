...
#include <cstring>
#include <vector>
#include <conio.h> // If you're on VS Code without DOSBox, replace this with system("pause") or use cin.get()
using namespace std;
...
void findclosure(int z, char a) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < strlen(clos[noitem][i].rhs); j++) {
            if (clos[noitem][i].rhs[j] == '.' && isvariable(clos[noitem][i].rhs[j + 1]) > 0) {
                for (int k = 0; k < novar; k++) {
...
int main() {
    ...
    for (int z = 0; z < noitem; z++) {
        cout << "\nI" << z << "\n";
        for (j = 0; j < arr[z]; j++)
            cout << clos[z][j].lhs << "->" << clos[z][j].rhs << endl;
        cout << "--------------------------" << endl;
    }
    cin.get();
    return 0;
